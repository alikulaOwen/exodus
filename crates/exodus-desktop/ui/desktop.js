    ...
    for (const item of currentItems) {
      ...
      if (state === 'contract_verified' || state === 'contractverified' || state === 'sandboxed') {
        ...
        await invoke('approve_operation', {
          id: item.id,
          approver: 'exodus-autonomous-pipeline',
          notes: 'Auto-verified and approved via pipeline runner'
        });
        await fetchQueue();
        await new Promise(r => setTimeout(r, 1200));
      }
...
function renderKanbanBoard() {
  const filtered = boardFilter === 'all'
    ? currentItems
    : currentItems.filter(i => i.domain_tag === boardFilter);
  ...
  Object.keys(buckets).forEach(stage => {
    ...
    // Attach drag & drop to column wrapper
...
function renderKanbanCard(item, stage) {
  ...
  // Issue identifier key (e.g. OP-104)
...
window.openCardDetail = (id) => {
  ...
  if (item) {
    selectItem(item);
    // Switch to Task & Review tab to inspect full details
...
window.advanceTask = async (id, action, event) => {
  ...
  try {
    if (action === 'verify') {
      await invoke('verify_operation', { id });
      showToast('Tests executed and verified in sandbox', 'success');
    } else if (action === 'approve') {
      await invoke('approve_operation', {
        id,
        approver: 'human_operator',
        notes: 'Approved via Kanban Board'
      });
  ...
  try {
    if (targetStage === 'verified' || targetStage === 'sandboxed') {
      ...
      await invoke('approve_operation', {
        id: itemId,
        approver: 'human_operator',
        notes: `Moved to ${targetStage} via Kanban drag & drop`
      });
      showToast(`Task moved to ${formatStateLabel(targetStage)}`, 'success');
    }
    await fetchQueue();
